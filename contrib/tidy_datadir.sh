#!/bin/bash
# Copyright (c) 2013 The Hashtagcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Hashtagcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Hashtagcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Hashtagcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Hashtagcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Hashtagcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Hashtagcoin 0.8 datadir."
    ;;
esac
