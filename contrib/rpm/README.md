...
The RPM spec file provided here is for Hashtagcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://hashtagcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Hashtagcoin git repository](https://github.com/hashtagcoin/hashtagcoin) but are not part of
...
Currently only `Source10` is used, for the example `hashtagcoin.conf` file.
...
in the hashtagcoin git repository. At present only an SVG version of the Hashtagcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the hashtagcoin
...
needed when 0.12.1 is released, a proper fix is already in the Hashtagcoin git
master branch.
...
idea to build Hashtagcoin Core against a static version of that library compiled
...
The `hashtagcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `hashtagcoin-server` package. At this point, most RPM based
...
The `hashtagcoin-server` package should have SELinux support. How to properly do
...
Hashtagcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/hashtagcoin/). He has been
packaging Hashtagcoin for Fedora at least since 2012.
...
4. I renamed the `hashtagcoin` package that contains the Qt GUI to `hashtagcoin-core` as
that appears to be how the general population refers to it, in contrast to
`hashtagcoin-xt` or `hashtagcoin-classic`. I wanted to make sure the general population
