...
*Setup instructions for a Gitian build of Hashtagcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Hashtagcoin
...
- [Building Hashtagcoin Core](#building-hashtagcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-hashtagcoin-host/README.md).
...
Clone the git repositories for hashtagcoin and Gitian.
...
in the hashtagcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Hashtagcoin Core
----------------

To build Hashtagcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the hashtagcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/hashtagcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/hashtagcoin/hashtagcoin
...
For example:
```bash
URL=https://github.com/laanwj/hashtagcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit hashtagcoin=${COMMIT} --url hashtagcoin=${URL} ../hashtagcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit hashtagcoin=${COMMIT} --url hashtagcoin=${URL} ../hashtagcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit hashtagcoin=${COMMIT} --url hashtagcoin=${URL} ../hashtagcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the hashtagcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../hashtagcoin/contrib/gitian-descriptors/*|sort|uniq )
...
HASHPATH=/some/root/path/hashtagcoin
SIGPATH=/some/root/path/hashtagcoin-detached-sigs

./bin/gbuild --url hashtagcoin=${HASHPATH},signature=${SIGPATH} ../hashtagcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hashtagcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hashtagcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hashtagcoin-osx-build.assert
```
...
[hashtagcoin-core/gitian.sigs](https://github.com/hashtagcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
